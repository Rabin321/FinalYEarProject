...
class MultinomialNaiveBayesNew:
    ...
    def fit(self, X, y):
        ...
        for i, c in enumerate(self.classes):
            ...
            #np.sum function used to count the number of samples in the training set that belong to each class'c'.
            #y == c returns a boolean array that is True where the class label is equal to c, and False otherwise.
            #The np.sum function then counts the number of samples that belong to class c
            #divide ko case
            #Dividing this count by the total number of samples n_samples gives the probability of class c in the training set.
...
# UPDATED PREDICT()
    def predict(self, X):
        ...
        for i, c in enumerate(self.classes):
            #P(Toxic)
            prior = np.log(self.class_probabilities[i]) #natural logarithm  is used instead of the regular logarithm
